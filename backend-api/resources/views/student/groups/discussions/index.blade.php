@extends('layouts.student')

@section('title', 'Discussions - UniForum')

@section('page-title', 'Discussions')

@section('content')

<div>

    <!-- Header -->
    <div class="bg-white border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 py-8">
          <div class="max-w-7xl mx-auto px-6 py-12 text-blue">
            <a href="/student/groups"
               class="text-sm text-white-600 hover:underline">

                ← Back to Groups

            </a>
          </div>

            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-5">

                <div>
                    <h1 class="text-3xl font-bold text-slate-900">
                        Discussions
                    </h1>

                    <p class="mt-2 text-slate-500">
                        Ask questions, share knowledge, and connect with fellow students.
                    </p>
                </div>

               @if($isMember)
                <a href="{{ route('student.discussions.create', $group->group_id) }}"
                   class="inline-flex items-center justify-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-xl hover:bg-blue-700 transition">

                    <svg class="w-5 h-5"
                         fill="none"
                         stroke="currentColor"
                         stroke-width="2"
                         viewBox="0 0 24 24">

                        <path stroke-linecap="round"
                              stroke-linejoin="round"
                              d="M12 4v16m8-8H4"/>
                    </svg>

                    Create Discussion

                </a>
                @endif

            </div>


            
            <!-- Search Form -->
            <div class="mt-8">
                <form method="GET" action="{{ route('student.discussions.index', $group->group_id) }}">
                    <div class="relative">
                        <input type="text"
                            name="search"
                            value="{{ request('search') }}"
                            placeholder="Search discussions by title or description..."
                            class="w-full rounded-xl border-slate-200 pl-12 pr-24 py-3 focus:ring-blue-500 focus:border-blue-500">

                        <svg class="absolute left-4 top-3.5 w-5 h-5 text-slate-400"
                            fill="none"
                            stroke="currentColor"
                            stroke-width="2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round"
                                stroke-linejoin="round"
                                d="m21 21-4.35-4.35m1.35-5.65a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>

                        @if(request('search'))
                            <a href="{{ route('student.discussions.index', $group->group_id) }}" 
                            class="absolute right-20 top-3 text-xs text-slate-400 hover:text-slate-600 bg-slate-100 px-2 py-1.5 rounded-lg">
                                Clear
                            </a>
                        @endif

                        <button type="submit" class="absolute right-2.5 top-2 bg-blue-600 text-white text-xs px-4 py-2 rounded-lg hover:bg-blue-700">
                            Search
                        </button>
                    </div>
                </form>
            </div>

        </div>
    </div>


    <!-- Recommended Discussions (AI-powered, top of page) -->
    @if(!empty($recommendedDiscussions) && $recommendedDiscussions->isNotEmpty())
    <div class="bg-slate-50 border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 py-8">

            <div class="flex items-center gap-2 mb-4">
                <span class="text-lg">✨</span>
                <h2 class="text-lg font-semibold text-slate-900">Recommended for you</h2>
            </div>

            <div class="flex gap-4 overflow-x-auto pb-2">
                @foreach($recommendedDiscussions as $topic)
                    <a href="{{ route('student.discussions.show', [$topic->group_id, $topic->topic_id]) }}"
                       class="min-w-[260px] max-w-[260px] flex-shrink-0 bg-white rounded-xl border border-slate-200 p-4 hover:shadow-md transition">

                        <span class="text-[11px] font-semibold bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full">
                            {{ $topic->ml_category ?? 'General' }}
                        </span>

                        <h3 class="mt-3 text-sm font-semibold text-slate-900 line-clamp-2">
                            {{ $topic->title }}
                        </h3>

                    </a>
                @endforeach
            </div>

        </div>
    </div>
    @endif


    <!-- Main Content -->

    <div class="max-w-7xl mx-auto px-6 py-10">

        <div class="grid lg:grid-cols-3 gap-8">


            <!-- Discussions -->

            <div class="lg:col-span-2 space-y-5">


                <!-- Discussion Card -->
                @forelse($topics as $topic)

                <div class="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md transition">


                    <div class="flex justify-between items-start">

                        <div>

                            <span class="text-xs font-semibold bg-blue-100 text-blue-700 px-3 py-1 rounded-full">
                                {{ $topic->ml_category ?? 'General' }}
                            </span>


                            <h2 class="mt-4 text-xl font-semibold text-slate-900">
                                {{ $topic->title }}
                            </h2>


                        </div>


                        <span class="text-sm text-slate-400">
                           {{ $topic->created_at->diffForHumans() }}
                        </span>


                    </div>


                    <div class="mt-5 flex items-center justify-between text-sm text-slate-500">

                        <span>
                            {{ $topic->messages_count }} replies
                        </span>


                        <a href="{{ route('student.discussions.show', [$group->group_id, $topic->topic_id]) }}"
                           class="text-blue-600 hover:underline">

                            View Discussion

                        </a>

                    </div>


                </div>
            @empty
                <div class="bg-white rounded-2xl border border-slate-200 p-6 text-slate-500 text-center">
                    @if(request('search'))
                        No discussions found matching "<strong>{{ request('search') }}</strong>".
                    @else
                        No discussions yet.
                    @endif
                </div>
            @endforelse


            <!-- Sidebar -->

            <div class="space-y-6">


                <!-- Categories -->

                <div class="bg-white rounded-2xl border border-slate-200 p-6">

                    <h3 class="font-semibold text-lg text-slate-900">
                        Popular Topics
                    </h3>


                    <div class="mt-4 space-y-3">
                     @forelse($popularCategories as $category)

                        <div class="flex justify-between">
                            <span class="text-slate-600">
                                {{ $category->ml_category }}
                            </span>
                            <span class="text-blue-600">
                                {{ $category->total }}
                            </span>
                        </div>
                        @empty
                            <p class="text-sm text-slate-500">
                                No categorized discussions yet.
                            </p>
                        @endforelse


                    </div>

                </div>



                <!-- Recommendation teaser -->


                <div class="bg-blue-600 rounded-2xl p-6 text-white">


                    <h3 class="font-semibold text-lg">
                        AI Recommendations
                    </h3>


                    <p class="mt-2 text-blue-100 text-sm">
                        Receive personalized discussion recommendations based on your previous engagement and interests.
                    </p>


                    <a href="/student/recommendations"
                       class="inline-block mt-4 bg-white text-blue-600 px-4 py-2 rounded-lg text-sm font-medium">
                        Explore
                    </a>


                </div>


            </div>


        </div>

    </div>


</div>


@endsection
