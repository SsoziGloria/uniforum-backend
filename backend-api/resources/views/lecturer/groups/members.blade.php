            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="border-t">
                    ...
                    <td class="px-6 py-5 space-x-3 text-sm">
                        {{-- Cannot demote or restrict owner or oneself --}}
                        @if(($member->user_id ?? $member->id) !== $group->created_by && ($member->user_id ?? $member->id) !== auth()->id())
                            ...
                            @if(($member->role ?? optional($member->pivot)->role) === 'member')
                                <form method="POST" action="{{ route('lecturer.groups.members.promote', ['group' => $group->group_id ?? $group->id, 'user' => $member->user_id ?? $member->id]) }}" class="inline">
                                    @csrf
                            ...
                            @elseif(($member->role ?? optional($member->pivot)->role) === 'admin')
                                <form method="POST" action="{{ route('lecturer.groups.members.demote', ['group' => $group->group_id ?? $group->id, 'user' => $member->user_id ?? $member->id]) }}" class="inline">
                                    @csrf
                            ...
                            @if(($member->warning_count ?? 0) < 2 && !($member->blacklisted_until && \Carbon\Carbon::parse($member->blacklisted_until)->isFuture()))
                                <form method="POST" action="{{ route('lecturer.groups.members.warning', ['group' => $group->group_id ?? $group->id, 'user' => $member->user_id ?? $member->id]) }}" class="inline">
                                    @csrf
                            ...
                            @if(!$member->blacklisted_until || !\Carbon\Carbon::parse($member->blacklisted_until)->isFuture())
                                <form method="POST" action="{{ route('lecturer.groups.members.blacklist', ['group' => $group->group_id ?? $group->id, 'user' => $member->user_id ?? $member->id]) }}" class="inline">
                                    @csrf
                            ...
                            @else
                                <form method="POST" action="{{ route('lecturer.groups.members.reinstate', ['group' => $group->group_id ?? $group->id, 'user' => $member->user_id ?? $member->id]) }}" class="inline">
                                    @csrf
